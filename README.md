...
## Chatting 10/13
    기존에 채팅에 접속해도 채팅 하나를 보내야(엔터를 눌러야) 정상적으로 비동기 recv가 되었음.
    즉 접속만 하고 아무 채팅도 하지않으면 메세지가 정상적으로 출력 X
    한 번 입력을 시작하면 그동안 받았던 메세지가 한 번에 출력되는 문제가 있었음.
    ...
    따라서 기존의 생성자에 있던 `start_write()`이후에 `io_context.run()`을 실행하는 함수인 `start_write_thread()`를 만들고
    `start_write_thread()`는 스레드의 시작 함수로 넣어서 만들자마자 바로 `detach()`를 통해서 동작하게 함.
